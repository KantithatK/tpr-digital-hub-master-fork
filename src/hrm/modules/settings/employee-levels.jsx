...
// Assumed Supabase table: employee_level (id uuid pk, level_id text unique, level_name_th text, level_name_en text, description text, is_active bool, created_at timestamptz, updated_at timestamptz)
// Adjust field/table names if they differ in your DB.
...
const initialForm = { id: '', level_id: '', level_name_th: '', level_name_en: '', description: '', is_active: true };
...
export default function EmployeeLevelsPage() {
  ...
  const [searchField, setSearchField] = React.useState('level_id');
  const [searchInput, setSearchInput] = React.useState('');
  const [appliedSearch, setAppliedSearch] = React.useState({ field: 'level_id', text: '' });
  ...
  const fetchData = React.useCallback(async () => {
    setLoading(true);
    try {
      let query = supabase.from('employee_level').select('*', { count: 'exact' });
  ...
  const handleOpenEdit = (row) => { setEditMode(true); setForm({ id: row.id, level_id: row.level_id || '', level_name_th: row.level_name_th || '', level_name_en: row.level_name_en || '', description: row.description || '', is_active: Boolean(row.is_active) }); setOriginalId(row.id); setErrors({}); setOpenModal(true); };
  ...
  const handleConfirmDelete = async () => { if (!targetRow || deleting) return; setDeleting(true); try { const { error } = await supabase.from('employee_level').delete().eq('id', targetRow.id); if (error) throw error; setSnackbar({ open: true, message: 'ลบข้อมูลสำเร็จ', severity: 'success' }); handleCloseDelete(); fetchData(); } catch (err) { setSnackbar({ open: true, message: err.message || 'ไม่สามารถลบข้อมูลได้', severity: 'error' }); } finally { setDeleting(false); } };

  const setField = (k, v) => { setForm((p) => ({ ...p, [k]: v })); setErrors((e) => ({ ...e, [k]: '' })); };
  const handleSave = async () => {
    if (saving) return;
    const newErrors = {};
    if (!form.level_id.trim()) newErrors.level_id = 'กรุณากรอกรหัสระดับพนักงาน';
    if (!form.level_name_th.trim()) newErrors.level_name_th = 'กรุณากรอกชื่อระดับพนักงาน (ไทย)';
    if (form.level_id.length > 20) newErrors.level_id = 'ไม่เกิน 20 ตัวอักษร';
    if (form.level_name_th.length > 150) newErrors.level_name_th = 'ไม่เกิน 150 ตัวอักษร';
    if (form.level_name_en && form.level_name_en.length > 150) newErrors.level_name_en = 'ไม่เกิน 150 ตัวอักษร';
    ...
    const payload = {
      ...
      level_name_th: form.level_name_th.trim(),
      level_name_en: form.level_name_en?.trim() || null,
    ...
    try {
      if (editMode) {
        const { error } = await supabase.from('employee_level').update(payload).eq('id', originalId);
        if (error) throw error;
        setSnackbar({ open: true, message: 'บันทึกการแก้ไขสำเร็จ', severity: 'success' });
      } else {
        const { error } = await supabase.from('employee_level').insert([{ ...payload, created_at: new Date().toISOString() }]);
        if (error) throw error;
      ...
      if (msg.toLowerCase().includes('duplicate') || msg.includes('unique') || err?.code === '23505') {
        setErrors((p) => ({ ...p, level_id: 'รหัสระดับพนักงานนี้ถูกใช้แล้ว' }));
      }
  ...
  return (
    <Box sx={{ p: { xs: 1.5, md: 2 } }}>
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 2 }}>
        <Typography sx={{ fontWeight: 'bold' }} variant="h6">กำหนดระดับพนักงาน</Typography>
      ...
      <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5} alignItems={{ xs: 'stretch', md: 'center' }} sx={{ mb: 2 }}>
        <FormControl sx={{ minWidth: 220 }} size="small">
          <InputLabel id="emp-level-search-field-label">ค้นหาตาม</InputLabel>
          <Select
            labelId="emp-level-search-field-label"
            label="ค้นหาตาม"
            value={searchField}
            onChange={(e) => setSearchField(e.target.value)}
          >
            <MenuItem value="level_id">รหัสระดับพนักงาน</MenuItem>
            <MenuItem value="level_name_th">ชื่อระดับพนักงาน (ไทย)</MenuItem>
            <MenuItem value="level_name_en">ชื่อระดับพนักงาน (อังกฤษ)</MenuItem>
          </Select>
        </FormControl>
        <TextField
          size="small"
        ...
        <Stack direction="row" spacing={1}>
          ...
          <Button startIcon={<ClearIcon />} color="inherit" onClick={() => { setSearchInput(''); setAppliedSearch({ field: 'level_id', text: '' }); setSearchField('level_id'); setPage(0); }}>ล้าง</Button>
        </Stack>
      </Stack>
      ...
      <TableContainer sx={{ borderRadius: 2 }}>
        <Table size="small" sx={{ '& th, & td': { borderRight: '1px solid', borderColor: 'divider' }, '& th:last-of-type, & td:last-of-type': { borderRight: 'none' } }}>
          <TableHead>
            <TableRow>
              <TableCell sx={{ bgcolor: 'grey.200', fontWeight: 'bold', whiteSpace: 'nowrap' }}>รหัสระดับพนักงาน</TableCell>
              <TableCell sx={{ bgcolor: 'grey.200', fontWeight: 'bold' }}>ชื่อระดับพนักงาน (ไทย)</TableCell>
              <TableCell sx={{ bgcolor: 'grey.200', fontWeight: 'bold' }}>ชื่อระดับพนักงาน (อังกฤษ)</TableCell>
            ...
            {loading ? (
              ...
              rows.map((row) => (
                <TableRow key={row.id} hover>
                  <TableCell sx={{ whiteSpace: 'nowrap' }}>{row.level_id}</TableCell>
                  <TableCell>{row.level_name_th || ''}</TableCell>
                  <TableCell>{row.level_name_en || ''}</TableCell>
      ...
      <Dialog open={openModal} onClose={() => { if (saving) return; handleCloseModal(); }} disableEscapeKeyDown={saving} fullWidth maxWidth="sm">
        <DialogTitle>{editMode ? 'แก้ไขระดับพนักงาน' : 'เพิ่มระดับพนักงาน'}</DialogTitle>
        <DialogContent dividers>
          <Stack spacing={2} sx={{ mt: 0.5 }}>
            <TextField label="รหัสระดับพนักงาน" value={form.level_id} onChange={(e) => setField('level_id', e.target.value)} inputProps={{ maxLength: 20 }} required size="small" disabled={editMode} error={Boolean(errors.level_id)} helperText={errors.level_id} />
            <TextField label="ชื่อระดับพนักงาน (ไทย)" value={form.level_name_th} onChange={(e) => setField('level_name_th', e.target.value)} inputProps={{ maxLength: 150 }} required size="small" error={Boolean(errors.level_name_th)} helperText={errors.level_name_th} />
            <TextField label="ชื่อระดับพนักงาน (อังกฤษ)" value={form.level_name_en} onChange={(e) => setField('level_name_en', e.target.value)} inputProps={{ maxLength: 150 }} size="small" error={Boolean(errors.level_name_en)} helperText={errors.level_name_en} />
      ...
      <Dialog open={openDelete} onClose={() => { if (deleting) return; handleCloseDelete(); }} disableEscapeKeyDown={deleting} fullWidth maxWidth="xs">
        ...
        <DialogContent dividers>
          <Typography variant="body2">ต้องการลบรหัส "{targetRow?.level_id}" หรือไม่?</Typography>
        </DialogContent>
        <DialogActions>
