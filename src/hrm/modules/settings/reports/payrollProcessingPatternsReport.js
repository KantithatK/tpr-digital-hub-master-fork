...
export const title = 'รายงานกำหนดรูปแบบประมวลผลจากระบบเวลาเข้าสู่ระบบเงินเดือน';

// PDF options for A4 landscape
...
export const columns = [
  ...
  { header: 'จ่ายค่าแรงวันหยุดนักขัตฤกษ์', dataKey: 'holiday_pay_enabled' },
  { header: 'ไม่เกิน (วัน)', dataKey: 'holiday_max_days' },
  { header: 'ชื่อรายได้พนักงานรายวัน', dataKey: 'holiday_daily_wage_desc' },
  { header: 'ชื่อรายได้พนักงานชั่วโมง', dataKey: 'holiday_hourly_wage_desc' },
];
...
export async function fetchRows(filters = {}) {
  ...
  return (data || []).map((r) => ({
    code: r.code,
    name: r.name || '-',
    daily_wage_code: r.daily_wage_code || '-',
    daily_wage_desc: r.daily_wage_desc || '-',
    hourly_wage_code: r.hourly_wage_code || '-',
    ...
    holiday_emp_daily_enabled: r.holiday_emp_daily_enabled ? 'ใช่' : 'ไม่',
    holiday_daily_wage_code: r.holiday_daily_wage_code || '-',
    holiday_daily_wage_desc: r.holiday_daily_wage_desc || '-',
    holiday_emp_hourly_enabled: r.holiday_emp_hourly_enabled ? 'ใช่' : 'ไม่',
    holiday_hourly_wage_code: r.holiday_hourly_wage_code || '-',
