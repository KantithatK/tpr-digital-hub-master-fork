...
// Assumed table: user_group { id uuid pk, group_id text unique, group_name_th text, group_name_en text, description text, is_active bool, created_at timestamptz, updated_at timestamptz }
// Adjust field/table names to match actual DB.
...
const initialForm = { id: '', group_id: '', group_name_th: '', group_name_en: '', description: '', is_active: true };
...
export default function UserGroupsPage() {
  ...
  const [searchField, setSearchField] = React.useState('group_id');
  const [searchInput, setSearchInput] = React.useState('');
  const [appliedSearch, setAppliedSearch] = React.useState({ field: 'group_id', text: '' });
  ...
  const fetchData = React.useCallback(async () => {
    setLoading(true);
    try {
      let query = supabase.from('user_group').select('*', { count: 'exact' });
  ...
  const handleOpenEdit = (row) => { setEditMode(true); setForm({ id: row.id, group_id: row.group_id || '', group_name_th: row.group_name_th || '', group_name_en: row.group_name_en || '', description: row.description || '', is_active: Boolean(row.is_active) }); setOriginalId(row.id); setErrors({}); setOpenModal(true); };
  ...
  const handleConfirmDelete = async () => { if (!targetRow || deleting) return; setDeleting(true); try { const { error } = await supabase.from('user_group').delete().eq('id', targetRow.id); if (error) throw error; setSnackbar({ open: true, message: 'ลบข้อมูลสำเร็จ', severity: 'success' }); handleCloseDelete(); fetchData(); } catch (err) { setSnackbar({ open: true, message: err.message || 'ไม่สามารถลบข้อมูลได้', severity: 'error' }); } finally { setDeleting(false); } };

  const setField = (k, v) => { setForm((p) => ({ ...p, [k]: v })); setErrors((e) => ({ ...e, [k]: '' })); };
  const handleSave = async () => {
    if (saving) return;
    const newErrors = {};
    if (!form.group_id.trim()) newErrors.group_id = 'กรุณากรอกรหัสกลุ่มผู้ใช้';
    if (!form.group_name_th.trim()) newErrors.group_name_th = 'กรุณากรอกชื่อกลุ่มผู้ใช้ (ไทย)';
    if (form.group_id.length > 20) newErrors.group_id = 'ไม่เกิน 20 ตัวอักษร';
    if (form.group_name_th.length > 150) newErrors.group_name_th = 'ไม่เกิน 150 ตัวอักษร';
    if (form.group_name_en && form.group_name_en.length > 150) newErrors.group_name_en = 'ไม่เกิน 150 ตัวอักษร';
    ...
    const payload = {
      ...
      group_name_th: form.group_name_th.trim(),
      group_name_en: form.group_name_en?.trim() || null,
    ...
    try {
      if (editMode) {
        const { error } = await supabase.from('user_group').update(payload).eq('id', originalId);
        if (error) throw error;
        setSnackbar({ open: true, message: 'บันทึกการแก้ไขสำเร็จ', severity: 'success' });
      } else {
        const { error } = await supabase.from('user_group').insert([{ ...payload, created_at: new Date().toISOString() }]);
        if (error) throw error;
      ...
      if (msg.toLowerCase().includes('duplicate') || msg.includes('unique') || err?.code === '23505') {
        setErrors((p) => ({ ...p, group_id: 'รหัสกลุ่มผู้ใช้นี้ถูกใช้แล้ว' }));
      }
  ...
  return (
    <Box sx={{ p: { xs: 1.5, md: 2 } }}>
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 2 }}>
        <Typography sx={{ fontWeight: 'bold' }} variant="h6">กำหนดกลุ่มผู้ใช้งาน</Typography>
      ...
      <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5} alignItems={{ xs: 'stretch', md: 'center' }} sx={{ mb: 2 }}>
        <FormControl sx={{ minWidth: 220 }} size="small">
          <InputLabel id="user-group-search-field-label">ค้นหาตาม</InputLabel>
          <Select
            labelId="user-group-search-field-label"
            label="ค้นหาตาม"
            value={searchField}
            onChange={(e) => setSearchField(e.target.value)}
          >
            <MenuItem value="group_id">รหัสกลุ่มผู้ใช้งาน</MenuItem>
            <MenuItem value="group_name_th">ชื่อกลุ่มผู้ใช้งาน (ไทย)</MenuItem>
            <MenuItem value="group_name_en">ชื่อกลุ่มผู้ใช้งาน (อังกฤษ)</MenuItem>
          </Select>
        </FormControl>
        <TextField
          size="small"
        ...
        <Stack direction="row" spacing={1}>
          ...
          <Button startIcon={<ClearIcon />} color="inherit" onClick={() => { setSearchInput(''); setAppliedSearch({ field: 'group_id', text: '' }); setSearchField('group_id'); setPage(0); }}>ล้าง</Button>
        </Stack>
      </Stack>
      ...
      <TableContainer sx={{ borderRadius: 2 }}>
        <Table size="small" sx={{ '& th, & td': { borderRight: '1px solid', borderColor: 'divider' }, '& th:last-of-type, & td:last-of-type': { borderRight: 'none' } }}>
          <TableHead>
            <TableRow>
              <TableCell sx={{ bgcolor: 'grey.200', fontWeight: 'bold', whiteSpace: 'nowrap' }}>รหัสกลุ่มผู้ใช้งาน</TableCell>
              <TableCell sx={{ bgcolor: 'grey.200', fontWeight: 'bold' }}>ชื่อกลุ่มผู้ใช้งาน (ไทย)</TableCell>
              <TableCell sx={{ bgcolor: 'grey.200', fontWeight: 'bold' }}>ชื่อกลุ่มผู้ใช้งาน (อังกฤษ)</TableCell>
            ...
            {loading ? (
              ...
              rows.map((row) => (
                <TableRow key={row.id} hover>
                  <TableCell sx={{ whiteSpace: 'nowrap' }}>{row.group_id}</TableCell>
                  <TableCell>{row.group_name_th || ''}</TableCell>
                  <TableCell>{row.group_name_en || ''}</TableCell>
      ...
      <Dialog open={openModal} onClose={() => { if (saving) return; handleCloseModal(); }} disableEscapeKeyDown={saving} fullWidth maxWidth="sm">
        <DialogTitle>{editMode ? 'แก้ไขกลุ่มผู้ใช้งาน' : 'เพิ่มกลุ่มผู้ใช้งาน'}</DialogTitle>
        <DialogContent dividers>
          <Stack spacing={2} sx={{ mt: 0.5 }}>
            <TextField label="รหัสกลุ่มผู้ใช้งาน" value={form.group_id} onChange={(e) => setField('group_id', e.target.value)} inputProps={{ maxLength: 20 }} required size="small" disabled={editMode} error={Boolean(errors.group_id)} helperText={errors.group_id} />
            <TextField label="ชื่อกลุ่มผู้ใช้งาน (ไทย)" value={form.group_name_th} onChange={(e) => setField('group_name_th', e.target.value)} inputProps={{ maxLength: 150 }} required size="small" error={Boolean(errors.group_name_th)} helperText={errors.group_name_th} />
            <TextField label="ชื่อกลุ่มผู้ใช้งาน (อังกฤษ)" value={form.group_name_en} onChange={(e) => setField('group_name_en', e.target.value)} inputProps={{ maxLength: 150 }} size="small" error={Boolean(errors.group_name_en)} helperText={errors.group_name_en} />
      ...
      <Dialog open={openDelete} onClose={() => { if (deleting) return; handleCloseDelete(); }} disableEscapeKeyDown={deleting} fullWidth maxWidth="xs">
        ...
        <DialogContent dividers>
          <Typography variant="body2">ต้องการลบรหัส "{targetRow?.group_id}" หรือไม่?</Typography>
        </DialogContent>
        <DialogActions>
